...
import { ArrowLeft, Clover, Calculator } from "lucide-react";
import Link from "next/link";
import AdBanner from "@/components/ads/AdBanner";
import AffiliateBlock from "@/components/ads/AffiliateBlock";

export default function LuckyCalculator() {
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <motion.div initial={ opacity: 0, y: 20 } animate={ opacity: 1, y: 0 } className="text-center mb-10">
          <div className="inline-flex items-center gap-2 rounded-full bg-primary-500/10 px-4 py-2 text-sm font-medium text-primary-400 mb-4">
            <Clover className="h-4 w-4" /> Sizning omadli raqamingiz
          </div>
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3">
            Omadli <span className="gradient-text">Raqam</span>
          </h1>
          <p className="text-gray-400 max-w-xl mx-auto">Sizning omadli raqamingiz</p>
        </motion.div>
        ...
        <motion.div initial={ opacity: 0, scale: 0.95 } animate={ opacity: 1, scale: 1 } className="glass-card p-6 mb-8">
          <form onSubmit={handleCalculate} className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" name="name" placeholder="Masalan: Aziz" className="input-field" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Tug'ilgan kun</label>
              <input type="text" name="birthdate" placeholder="YYYY-MM-DD" className="input-field" />
            </div>
