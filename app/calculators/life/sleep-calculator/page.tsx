...
import { ArrowLeft, Bed, Calculator } from "lucide-react";
import Link from "next/link";
import AdBanner from "@/components/ads/AdBanner";
import AffiliateBlock from "@/components/ads/AffiliateBlock";

export default function SleepcalcCalculator() {
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <motion.div initial={ opacity: 0, y: 20 } animate={ opacity: 1, y: 0 } className="text-center mb-10">
          <div className="inline-flex items-center gap-2 rounded-full bg-primary-500/10 px-4 py-2 text-sm font-medium text-primary-400 mb-4">
            <Bed className="h-4 w-4" /> Qachon uxlash kerak
          </div>
          <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3">
            Uyqi <span className="gradient-text">Hisoblagich</span>
          </h1>
          <p className="text-gray-400 max-w-xl mx-auto">Qachon uxlash kerak</p>
        </motion.div>
        ...
        <motion.div initial={ opacity: 0, scale: 0.95 } animate={ opacity: 1, scale: 1 } className="glass-card p-6 mb-8">
          <form onSubmit={handleCalculate} className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Uyg'onish vaqti</label>
              <input type="text" name="wake" placeholder="Masalan: 07:00" className="input-field" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Uyqi tsikllari</label>
              <input type="text" name="cycles" placeholder="Masalan: 5" className="input-field" />
            </div>
